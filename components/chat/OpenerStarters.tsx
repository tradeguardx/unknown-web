...
// stranger who did NOT open first, so the thread would otherwise sit blank. Gives
// a few tappable conversation starters — tapping one sends it as the user's first
// message. Turns dead air into engagement (cold-start is a top skip cause).

export function OpenerStarters({ onPick }: { onPick: (text: string) => void }) {
  return (
    <div className="flex-1 overflow-y-auto px-6 py-8 flex flex-col items-center justify-center text-center">
      ...
      <p className="mt-2 max-w-[16rem] font-sans text-[13px] leading-relaxed text-ink-mute">
        no names, no history. it vanishes when you leave. just say hi:
      </p>
